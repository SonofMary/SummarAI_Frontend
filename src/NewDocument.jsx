...
function NewDocument() {
  ...
  const handleNext = () => {
    ...
    // Update score if correct - comparing full text
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      const response = await axios.post(
        "https://summarai-backend.onrender.com/summarai/quiz/upload",
        userQuizData,
        {
          headers: {
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(parsedQuiz) && parsedQuiz.length > 0) {
          ...
          setError("Quiz came back empty! Try a different file.");
        }
      } catch (parseError) {
  ...
  const chatWithAi = async () => {
    ...
    try {
      const response = await axios.post(
        "https://summarai-backend.onrender.com/summarai/chat",
        { message: userPrompt }
      );
      ...
      {activeSection === "upload" && !selectedFile && !loadingSummary && (
        <div className="flex flex-col items-center justify-center gap-4 text-center">
          ...
          <p className="text-gray-600 max-w-md">
            Upload a PDF or DOCX file to generate summaries, quizzes, and chat with AI.
          </p>
